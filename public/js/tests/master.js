...
test('Create seat sections', function(){
  ...
  deepEqual($('.seatGeneral').length, 200, 'after first click, GA grid should have 200 seats');
  deepEqual($('#general > div').length, 1, 'after first click, should be one grid');
  deepEqual($('.seatGeneral:last-child > p').text(), 'GA200', 'after first click, p in last GA seat should have value of GA200'); // if fails, try no parens on 200
  ...
  $('#price').val('65');
  $('#createSeats').trigger('click');
  deepEqual($('.seatVip').length, 60, 'after second click, VIP grid should have 60 seats');
  deepEqual($('#vip > div').length, 2, 'after second click, should be two grids');
  deepEqual($('.seatVip:last-child > p').text(), 'VIP60', 'after second click, p in last VIP seat should have value of VIP60'); // if fails, try no parens on 200
